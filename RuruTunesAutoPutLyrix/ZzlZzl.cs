    ...
    internal class ZzlZzl
    {
        ...
        private string getCover(string query, bool bigsize)
        {
            ...
            try
            {
                ...
                string str2 = "http://cover.zzlzzl.net/";
                string text = this.HttpPost(str2 + "?" + query, "");
                foreach (string str4 in this.returnSubstrings(text, "<tr>", "</tr>"))
                {
                    ...
                    if ((str5.ToUpper().IndexOf(this.m_artist.ToUpper()) > 0) && (str5.ToUpper().IndexOf(this.m_album.ToUpper()) > 0))
                    {
                        ...
                        text = this.HttpPost(str2 + strArray2[0], query);
                        string[] strArray3 = this.returnSubstrings(text, "<img src='", "'");
                        if (strArray3.Length > 0)
                        {
                            string fileName = Path.GetFileName(strArray3[0]);
                            ...
                            {
                                client.DownloadFile(strArray3[0], str7);
                            }
                            if (!System.IO.File.Exists(str7))
                            {
                                client.DownloadFile(strArray3[0], str7);
        ...
        public string getCover(string artist, string album, string song, bool bigsize, bool AlbumTitleOnly)
        {
            ...
            if ((album != "") && (album != null))
            {
                this.m_form.Invoke(this.m_form.m_SetStatus, new object[] { "즐즐넷 앨범커버 : '" + this.m_album + "' by '" + this.m_artist + "' 검색중" });
                query = "mode=search&Submit=Submit&search_str=" + artist;
                if (str == null)
                {
            ...
            if (str == null)
            {
                query = "mode=search&Submit=Submit&search_str=" + album;
                this.m_form.Invoke(this.m_form.m_SetStatus, new object[] { "즐즐넷 앨범커버 : '" + this.m_song + "' by '" + this.m_artist + "' 검색중" });
                str = this.getCover(query, bigsize);
            }
            if ((AlbumTitleOnly && (album != "")) && ((album != null) && (str == null)))
            {
                query = "mode=search&Submit=Submit&search_str=" + album;
                this.m_form.Invoke(this.m_form.m_SetStatus, new object[] { "즐즐넷 앨범커버 : '" + this.m_album + "' 검색중 ( Experimental )" });
                str = this.getCover(query, bigsize);
            }
            return str;
        }
