namespace RuruTunesAutoPutLyrix
{
    ...
    internal class Gasazip
    {
        public MassUpdater m_form;

        public bool checkSongExists(string artist, string song)
        {
            string uri = "http://gasazip.com/search.html";
            string text = this.HttpPost(uri, "word=" + song);
            if (text != null)
            {
                foreach (string str3 in this.returnSubstrings(text, "/view.html?", "</a>"))
                {
                ...
                string[] strArray2 = this.returnSubstrings(text, "/search.html?", "&amp;");
                if (strArray2.Length > 0)
                {
                    int num = 0;
                    foreach (string str4 in strArray2)
                    {
                        text = this.HttpPost(uri, str4 + "&word=" + song);
                        foreach (string str5 in this.returnSubstrings(text, "/view.html?", "</a>"))
                        {
                ...
                uri = "http://gasazip.com/search.html";
                text = this.HttpPost(uri, "word=" + artist);
                if (text == null)
                {
                    return false;
                }
                foreach (string str6 in this.returnSubstrings(text, "/view.html?", "</a>"))
                {
                ...
                strArray2 = this.returnSubstrings(text, "/search.html?", "&amp;");
                if (strArray2.Length > 0)
                {
                    int num2 = 0;
                    foreach (string str7 in strArray2)
                    {
                        text = this.HttpPost(uri, str7 + "&word=" + artist);
                        foreach (string str8 in this.returnSubstrings(text, "/view.html?", "</a>"))
                        {
        ...
        public string getSong(string artist, string song)
        {
            string uri = "http://gasazip.com/search.html";
            string text = this.HttpPost(uri, "word=" + song);
            string[] strArray = this.returnSubstrings(text, "/view.html?no=", "</a>");
            if (strArray.Length == 1)
            {
                uri = "http://gasazip.com/view.html";
                string parameters = "no=" + strArray[0].Substring(0, strArray[0].IndexOf('"'));
                text = this.HttpPost(uri, parameters);
                string[] strArray2 = this.returnSubstrings(text, "blackbb\">", "<td width=\"300\"");
                if (strArray2.Length > 0)
                {
            ...
            foreach (string str4 in strArray)
            {
                ...
                {
                    uri = "http://gasazip.com/view.html";
                    string str5 = "no=" + str4.Substring(0, str4.IndexOf('"'));
                    text = this.HttpPost(uri, str5);
                    string[] strArray3 = this.returnSubstrings(text, "blackbb\">", "<td width=\"300\"");
            ...
            string[] strArray4 = this.returnSubstrings(text, "/search.html?", "&amp;");
            if (strArray4.Length > 0)
            {
                int num = 0;
                foreach (string str6 in strArray4)
                {
                    text = this.HttpPost(uri, str6 + "&word=" + song);
                    foreach (string str7 in this.returnSubstrings(text, "/view.html?no=", "</a>"))
                    {
                        ...
                        {
                            uri = "http://gasazip.com/view.html";
                            string str8 = "no=" + str7.Substring(0, str7.IndexOf('"'));
                            text = this.HttpPost(uri, str8);
                            string[] strArray5 = this.returnSubstrings(text, "blackbb\">", "<td width=\"300\"");
                            if (strArray5.Length > 0)
                            {
                                return StripTags(strArray5[0]);
                            }
                            return null;
                        }
                    }
        ...
        {
            string uri = "http://gasazip.com/search.html";
            string text = this.HttpPost(uri, "word=" + artist);
            foreach (string str3 in this.returnSubstrings(text, "/view.html?no=", "</a>"))
            {
                if ((str3.Contains(song) || str3.Contains(song.Replace(" ", ""))) || (str3.Replace(" ", "").Contains(song) || str3.Replace(" ", "").Contains(song.Replace(" ", ""))))
                {
                    uri = "http://gasazip.com/view.html";
                    string parameters = "no=" + str3.Substring(0, str3.IndexOf('"'));
                    text = this.HttpPost(uri, parameters);
                    string[] strArray2 = this.returnSubstrings(text, "blackbb\">", "<td width=\"300\"");
                    if (strArray2.Length > 0)
                    {
            ...
            string[] strArray3 = this.returnSubstrings(text, "/search.html?", "&amp;");
            if (strArray3.Length > 0)
            {
                int num = 0;
                foreach (string str5 in strArray3)
                {
                    text = this.HttpPost(uri, str5 + "&word=" + artist);
                    foreach (string str6 in this.returnSubstrings(text, "/view.html?no=", "</a>"))
                    {
                        if ((str6.Contains(song) || str6.Contains(song.Replace(" ", ""))) || (str6.Replace(" ", "").Contains(song) || str6.Replace(" ", "").Contains(song.Replace(" ", ""))))
                        {
                            uri = "http://gasazip.com/view.html";
                            string str7 = "no=" + str6.Substring(0, str6.IndexOf('"'));
                            text = this.HttpPost(uri, str7);
                            string[] strArray4 = this.returnSubstrings(text, "blackbb\">", "<td width=\"300\"");
                            if (strArray4.Length > 0)
                            {
