namespace TagGuru
{
    ...
    internal class AutoUpdate
    {
        ...
        private TagGuru tagGuru;

        public AutoUpdate(TagGuru tg)
        {
            this.tagGuru = tg;
            string text = this.tagGuru.lblVersion.Text;
            this.curVersion = this.returnSubstrings(text, "TagGuru ", " -")[0];
        ...
        public void UpdateMe()
        {
            try
            {
                ...
                if (((int.Parse(str3.Replace(".", "")) > int.Parse(this.curVersion.Replace(".", ""))) || (int.Parse(s) > int.Parse(this.curBuild))) && (MessageBox.Show("TagGuru 가 [" + str3 + " - " + s + "] 버전으로 업데이트 되었습니다." + Environment.NewLine + "새로운 버전으로 업데이트 하시겠습니까 ?" + Environment.NewLine + Environment.NewLine + "[확인]을 누르면 새 버전을 다운로드하고 설치를 시작합니다.", "TagGuru AutoUpdate", MessageBoxButtons.OKCancel) == DialogResult.OK))
                {
                    Directory.CreateDirectory(Path.GetTempPath() + @"\lyricguru\");
                    string path = Path.GetTempPath() + @"lyricguru\tagguru_" + str3 + s + ".exe";
