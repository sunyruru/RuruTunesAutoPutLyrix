namespace TagGuru
{
    ...
    public class TagGuru : Form
    {
        ...
        private void btnStart_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("동작을 선택하지 않았습니다. 가사,커버,Tag 중의 하나를 선택해주세요..", "TagGuru 오류");
            }
            else
            {
                ...
                {
                    MessageBox.Show("iTunes에 선택된 곡이 없습니다. 곡을 선택후 실행하여 주세요.", "TagGuru 오류");
                }
                ...
                else
                {
                    try
                    {
                        ...
                        {
                            MessageBox.Show("앨범크기를 잘못 입력하셨습니다.", "TagGuru 오류");
                            return;
                        }
                    }
                    catch (FormatException exception)
                    {
                        MessageBox.Show("앨범크기를 잘못 입력하셨습니다. : " + exception.Message, "TagGuru 오류");
                        return;
                    }
        ...
        {
            ComponentResourceManager manager = new ComponentResourceManager(typeof(TagGuru));
            ...
            this.lblVersion.Text = "TagGuru 1.2.8 - 20090910";
            ...
            this.Text = "TagGuru 1.2 - iTunes Lyrics / Album Cover Importer ";
            base.Load += new EventHandler(this.LyricGuru_Load);
            base.FormClosing += new FormClosingEventHandler(this.TagGuru_FormClosing);
        ...
        {
            Savior.Read(this.settings, @"Software\TagGuru\Settings");
        ...
        private void LyricGuru_Load(object sender, EventArgs e)
        {
            try
            {
                if (File.Exists(Application.StartupPath + @"\tagguru_log.txt"))
                {
                    File.Delete(Application.StartupPath + @"\tagguru_log.txt");
                }
            }
            catch (Exception)
            {
            ...
            try
            {
                AppDomain.CurrentDomain.UnhandledException += delegate (object sender2, UnhandledExceptionEventArgs e2) {
                    if (e2.IsTerminating)
                    {
                        ...
                        StreamWriter writer = new StreamWriter(Application.StartupPath + @"\tagguru_log.txt", true);
            ...
            {
                StreamWriter writer2 = new StreamWriter(Application.StartupPath + @"\tagguru_log.txt", true);
        ...
        private void SaveSettings()
        {
            ...
            Savior.Save(this.settings, @"Software\TagGuru\Settings");
        }

        private void TagGuru_FormClosing(object sender, FormClosingEventArgs e)
        {
            this.SaveSettings();
        }
    }
